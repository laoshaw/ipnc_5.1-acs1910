...
int AVSERVER_dmvaAppEnable(Bool enable)
{
  gAVSERVER_config.dmvaAppConfig.dmvaEnable 	= enable;
...
int AVSERVER_setVnfStrength(int strength)
{
  int i=0;

  for(i=0;i<AVSERVER_MAX_STREAMS;i++) {
  	gAVSERVER_config.captureConfig[i].vnfStrength  = strength;
    gAVSERVER_config.captureConfig[i].vnfPrmUpdate = TRUE;
  }
...
int AVSERVER_swosdDmvaText(char *strText, int nLength)
{
	int count = 0;
	if(nLength >= MAX_STRING_LENGTH)
		return OSA_EFAIL;
	...
	for(count = 0; count<AVSERVER_MAX_STREAMS; count++)
	{
		memcpy(gAVSERVER_config.captureConfig[count].swosdConfig.swosdDmvaDispString,
				strText, nLength);
		gAVSERVER_config.captureConfig[count].swosdConfig.swosdDmvaDispString[nLength] = '\0';
		gAVSERVER_config.captureConfig[count].swosdConfig.swosdDmvaStringUpdate = OSD_YES;
	}
	return OSA_SOK;
}
...
int AVSERVER_swosdDmvaEnableText(int enable)
{
	int count = 0;
	for(count = 0; count<AVSERVER_MAX_STREAMS;count++)
	{
		if(enable)
		{
			gAVSERVER_config.captureConfig[count].swosdConfig.swosdDmvaTextEnable = 1;
		}
		else
		{
			gAVSERVER_config.captureConfig[count].swosdConfig.swosdDmvaTextEnable = 0;
		}
	}
	return OSA_SOK;
}
...
int AVSERVER_SetDisplay(int type)
{
	...
	if( type == 2 /*PAL*/ ) {
		...
		if(status != OSA_SOK)
		{
			OSA_ERROR("DRV_displaySetMode() for NTSC\n");
		}
		...
		if(status != OSA_SOK)
		{
			OSA_ERROR("DRV_displaySetMode() for PAL\n");
		}
...
int AVSERVER_profileInfoShow()
{
  ...
  AVSERVER_profilePrint("DEI 	 ", &gAVSERVER_ctrl.deiPrf);
  ...
  AVSERVER_profilePrint("DMVA    ", &gAVSERVER_ctrl.dmvaPrf);
  ...
  AVSERVER_profilePrint("ENCRYPT0", &gAVSERVER_ctrl.encryptPrf[0]);
  AVSERVER_profilePrint("ENCRYPT1", &gAVSERVER_ctrl.encryptPrf[1]);
  AVSERVER_profilePrint("ENCRYPT2", &gAVSERVER_ctrl.encryptPrf[2]);
  AVSERVER_profilePrint("ENCRYPT3", &gAVSERVER_ctrl.encryptPrf[3]);
  AVSERVER_profilePrint("DECRYPT0", &gAVSERVER_ctrl.decryptPrf[0]);
  AVSERVER_profilePrint("DECRYPT1", &gAVSERVER_ctrl.decryptPrf[1]);
  AVSERVER_profilePrint("DECRYPT2", &gAVSERVER_ctrl.decryptPrf[2]);
  AVSERVER_profilePrint("DECRYPT3", &gAVSERVER_ctrl.decryptPrf[3]);
  OSA_printf("\n");
...
int AVSERVER_init()
{
  ...
  DRV_imgSetWidthHeightYuvInterlacedMode(gAVSERVER_UI_config.yuvModeWidth,gAVSERVER_UI_config.yuvModeHeight);
#endif
...
int AVSERVER_exit()
{
  ...
  #ifdef AVSERVER_DEBUG_API
  OSA_printf(" AVSERVER API: DMVA Delete ...\n");
  #endif

  status |= VIDEO_dmvaDelete();
...
void AVSERVER_WDT_RESET(void)
{
	...
	pRegAddr 	= DM365MM_mmap(0x01C20000, 0x2000);
    OSA_waitMsecs(100);

#ifdef UBL_WDT_RESET

	pReg 		= (unsigned int *)(pRegAddr+0x1C08);	// WDT
	*pReg  		= 0x00020000;
	pReg 		= (unsigned int *)(pRegAddr+0x1C0C);	// execute >
	*pReg  		= 0x00020002;

#else

	pReg 		= (unsigned int *)(pRegAddr+0x1C24);	// TGCR
	*pReg  		|= 0x0B;

	pReg 		= (unsigned int *)(pRegAddr+0x1C10);	// TIM12
	*pReg  		= 0x01;
	pReg 		= (unsigned int *)(pRegAddr+0x1C14);	// TIM34
	*pReg  		= 0x00;

	pReg 		= (unsigned int *)(pRegAddr+0x1C18);	// PRD12
	*pReg  		= 0x0FFFFFFF;
	pReg 		= (unsigned int *)(pRegAddr+0x1C1C);	// PRD34
	*pReg  		= 0x0;
