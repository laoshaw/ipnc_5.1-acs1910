...
int AVSERVER_tskConnectReset()
{
  ...
  for(i=0; i<gAVSERVER_config.numCaptureStream; i++) {
    ...
    pCaptureStream->deiNextTsk = VIDEO_TSK_NONE;
    ...
    pCaptureStream->dmvaCopyTsk = VIDEO_TSK_NONE;
    ...
    if( gAVSERVER_config.captureSingleResize && i>0 )
    {
      ...
      pCaptureStream->dmvaCopyTsk = VIDEO_TSK_RESIZE;
    ...
    } else
    {
      ...
      if(gAVSERVER_config.deiTskEnable) {
        if(gAVSERVER_config.ldcTskEnable)
          pCaptureStream->deiNextTsk = VIDEO_TSK_LDC;
	    else
	    if(gAVSERVER_config.vnfTskEnable)
          pCaptureStream->deiNextTsk = VIDEO_TSK_VNF;
        else
        if(gAVSERVER_config.captureSingleResize)
          pCaptureStream->deiNextTsk = VIDEO_TSK_RESIZE;
        else
          pCaptureStream->deiNextTsk = VIDEO_TSK_ENCODE;
      }
      ...
      pCaptureStream->dmvaCopyTsk = VIDEO_TSK_CAPTURE;
      pCaptureStream->vsApplyTsk = VIDEO_TSK_CAPTURE;
      if(gAVSERVER_config.deiTskEnable) {
        pCaptureStream->swosdRunTsk = VIDEO_TSK_DEI;
        pCaptureStream->displayCopyTsk = VIDEO_TSK_DEI;
        pCaptureStream->faceDetectCopyTsk = VIDEO_TSK_DEI;
        pCaptureStream->vsApplyTsk = VIDEO_TSK_DEI;
      }
      if(gAVSERVER_config.ldcTskEnable) {
        ...
        pCaptureStream->dmvaCopyTsk = VIDEO_TSK_LDC;
      ...
      if(gAVSERVER_config.vnfTskEnable) {
        ...
        pCaptureStream->dmvaCopyTsk = VIDEO_TSK_VNF;
      }
#ifdef YUV_MODE_INTERLACED
      if(gAVSERVER_config.captureSingleResize){
        pCaptureStream->displayCopyTsk = VIDEO_TSK_RESIZE;
  ...
  for(i=0; i<gAVSERVER_config.numEncodeStream; i++) {
    ...
    if(gAVSERVER_config.encryptTskEnable)
	{
	  ...
	  pEncodeStream->encryptNextTsk = VIDEO_TSK_STREAM;
	}
  ...
  *pBufHndl=NULL;
  ...
  if(tskId <= VIDEO_TSK_RESIZE) {
    ...
    if(tskId==VIDEO_TSK_DEI) {
  	  nextTskId = pCaptureStream->deiNextTsk;
    } else
    ...
    if(nextTskId==VIDEO_TSK_DEI) {
      *pBufHndl = &pCaptureStream->bufDeiIn;
      *pTskHndl = &gVIDEO_ctrl.deiTsk;
    } else
    ...
    if(tskId==VIDEO_TSK_ENCRYPT) {
      nextTskId = pEncodeStream->encryptNextTsk;
    }
