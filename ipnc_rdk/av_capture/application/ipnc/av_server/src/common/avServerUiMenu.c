...
int UI_menuRun()
{
  ...
  while(!done) {
   ...
   OSA_printf( " Advanced DMVA                   : ");
   if(gAVSERVER_UI_ctrl.dmvaEnable) {
      OSA_printf("ON");
    } else {
      OSA_printf("OFF");
    }
    ...
    switch(ch) {
	  ...
	  case '9':
        ...
        gAVSERVER_UI_ctrl.dmvaEnable = !gAVSERVER_UI_ctrl.dmvaEnable;
        AVSERVER_dmvaAppEnable(gAVSERVER_UI_ctrl.dmvaEnable);
