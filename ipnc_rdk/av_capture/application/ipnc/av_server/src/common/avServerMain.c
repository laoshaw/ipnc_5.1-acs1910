...
void showUsage()
{
  ...
  OSA_printf(" DMVA         : Advanced DM Video Analytics is ENABLED\n");
  ...
  OSA_printf(" ENCRYPT	    : AES Encryption is ENABLED\n");
  OSA_printf(" NTSC/PAL/NOFLICKER : Mode for Flicker Detect\n");
  OSA_printf(" <PLATFORM>   : DM365/DM368/DM369/DMVA1/DMVA2/DM368E to be slected based on the platform chip\n");
  OSA_printf("\n");
...
int main(int argc, char **argv)
{
  ...
  gAVSERVER_UI_config.dmvaEnable        = FALSE;
  ...
  gAVSERVER_UI_config.encryptEnable     = FALSE;
  ...
  for(i=1; i< argc; i++) {
    ...
    if(strcmp(argv[i], "D1")==0)
    {
      gAVSERVER_UI_config.mode = AVSERVER_UI_CAPTURE_MODE_D1;
      #ifdef YUV_MODE_INTERLACED
      gAVSERVER_UI_config.yuvModeWidth  = atoi(argv[i + 1]);
      gAVSERVER_UI_config.yuvModeHeight = atoi(argv[i + 2]);
      gAVSERVER_UI_config.aewbEnable = FALSE;
	  i += 2;
      #endif
  	}
...
///////////////////end add
	...
	if(strcmp(argv[i], "DMVA")==0)
	  gAVSERVER_UI_config.dmvaEnable = TRUE;
    else
    ...
    if(strcmp(argv[i], "DM368E")==0)
      gAVSERVER_UI_config.platform_id = PLATFORM_DM368E;
    else
    if(strcmp(argv[i], "DM369")==0)
      gAVSERVER_UI_config.platform_id = PLATFORM_DM369;
    else
    if(strcmp(argv[i], "DMVA1")==0)
	{
      gAVSERVER_UI_config.platform_id = PLATFORM_DMVA1;
	  gAVSERVER_UI_config.dmvaEnable  = TRUE;
	}
  	else
    if(strcmp(argv[i], "DMVA2")==0)
	{
      gAVSERVER_UI_config.platform_id = PLATFORM_DMVA2;
	  gAVSERVER_UI_config.dmvaEnable  = TRUE;
	}
    else
	...
	if(strcmp(argv[i], "ENCRYPT")==0)
	  gAVSERVER_UI_config.encryptEnable = TRUE;
  }
...
#ifdef BOARD_TI_EVM
#ifdef YUV_MODE_INTERLACED
    if(gAVSERVER_UI_config.yuvModeHeight == 480){
	  	AVSERVER_SetDisplay(1);
	}
	else if(gAVSERVER_UI_config.yuvModeHeight == 576){
		AVSERVER_SetDisplay(2);
	}
#else
	AVSERVER_SetDisplay(1);
#endif
