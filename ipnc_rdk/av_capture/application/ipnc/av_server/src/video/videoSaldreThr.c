...
static int *g_saldre_rszBufId;
static int g_saldre_numStreams;

static OSA_ThrHndl g_saldreThrHndl;

static Uint32 g_saldre_numframes= 0;
static Uint16 g_saldre_frameSkip= 0;
...
static Uint16 g_saldre_inputImageWidth;
static Uint16 g_saldre_inputImageHeight;
...
int VIDEO_saldreAllocBuffers(Uint16 width, Uint16 height)
{
  ...
  */
  ...
  g_saldre_weightBufVirtAddr = OSA_cmemAlloc(bufSize, 32); /* Note that bufSize= width*height and width's unit is number of bytes, not pixels. So no need of an extra x2 factor */
  g_saldre_boxcarVirtAddr = OSA_cmemAlloc((bufSize/256), 32); /* Note that bufSize= width*height and width's unit is number of bytes, not pixels.  So no need of an extra x2 factor */
...
int VIDEO_saldreRun(OSA_BufInfo **pOutBufInfo, int *rszBufId, int numStreams)
{
  ...
  Uint32 GBEStrength, GCEStrength, LBEStrength, LCEStrength;
  ...
  g_saldre_rszBufId= rszBufId;
  g_saldre_numStreams = numStreams;
  ...
  imgWidth= g_saldre_inputImageWidth;
  imgHeight= g_saldre_inputImageHeight;
  ...
  if (!(g_saldre_numframes % g_saldre_frameSkip))
  ...
  if(gAVSERVER_config.aewb_config.saldreLevel==SALDRE_HIGH){
    ...
    LBEStrength= 8192; LCEStrength= 4096;
  }
  else if(gAVSERVER_config.aewb_config.saldreLevel==SALDRE_MED) {
    GBEStrength= 128; GCEStrength= 64;
    LBEStrength= 6144; LCEStrength= 4096;
  }
  else { /* SALDRE_LOW */
    GBEStrength= 64; GCEStrength= 64;
    LBEStrength= 4096; LCEStrength= 4096;
  }

  if(gAVSERVER_config.aewb_config.saldreMode == SALDRE_ADAPTIVE)  {
	  getTheSmartGLBCEStrengths(gAVSERVER_config.aewb_config.saldreLevel, 1, gOutAWBData.sensorGain,
								gOutAWBData.awb_data.rGain, gOutAWBData.awb_data.grGain,
								gOutAWBData.awb_data.gbGain, gOutAWBData.awb_data.bGain,
								&GBEStrength, &GCEStrength, &LBEStrength, &LCEStrength);
  }
  else {
	  if(gAVSERVER_config.aewb_config.saldreMode != SALDRE_LOCAL)  {
	     LBEStrength= 0; LCEStrength=0;
	  }
	  else {
	    GBEStrength= 0; GCEStrength= 0;
	  }
  }

  //countframes = countframes+1;
  if (!(g_saldre_numframes % g_saldre_frameSkip))
    SALDRE_runFunc1(g_saldre_boxcarVirtAddr, saldreInBufferIndex, imgWidth, imgHeight, 0,GBEStrength, GCEStrength, LBEStrength, LCEStrength);
...
#ifdef SALDRE_BENCHMARK
  if (g_saldre_numframes % (100*g_saldre_frameSkip) == 0) {
...
int VIDEO_saldreCreate(Uint16 skip)
{
  int status;
  Uint16 stride;

  g_saldre_frameSkip= skip+1;
  ...
  g_saldre_inputImageWidth= (SALDRE_MAX_IMGWIDTH < gVIDEO_ctrl.captureInfo.isifInfo.ddrOutDataWidth ? SALDRE_MAX_IMGWIDTH : gVIDEO_ctrl.captureInfo.isifInfo.ddrOutDataWidth);
  g_saldre_inputImageHeight= (SALDRE_MAX_IMGHEIGHT < gVIDEO_ctrl.captureInfo.isifInfo.ddrOutDataHeight ? SALDRE_MAX_IMGHEIGHT : gVIDEO_ctrl.captureInfo.isifInfo.ddrOutDataHeight);

  SALDRE_create(g_saldre_inputImageWidth, g_saldre_inputImageHeight, 2);
...
#ifdef SALDRE_LOAD_TEST_FRAME
{
  g_saldre_testFrame= OSA_cmemAlloc(g_saldre_inputImageWidth*g_saldre_inputImageHeight*2, 32);
  if (g_saldre_testFrame== NULL) {
    OSA_ERROR("saldre: Unable to allocate memory for test frame\n");
  }
  else {
    OSA_printf("saldre: Loading test frame ...\n");
    load_frame("ipnc_raw_test.raw", g_saldre_testFrame, g_saldre_inputImageWidth*g_saldre_inputImageHeight*2);
