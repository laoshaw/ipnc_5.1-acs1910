...
int VIDEO_captureTskCreate()
{
  ...
  captureConfig.isifConfig.numBuf     = (gAVSERVER_UI_config.mode == AVSERVER_UI_CAPTURE_MODE_5_MEGA) ?
  											(VIDEO_NUM_BUF):(VIDEO_NUM_BUF + 1);   //increase by one to reduce bottleneck
  ...
  captureConfig.ipipeConfig.rszOutConfig[0].dmaInterval   = gAVSERVER_config.captureConfig[0].dmaInterval;
  captureConfig.ipipeConfig.rszOutConfig[1].dmaInterval   = gAVSERVER_config.captureConfig[1].dmaInterval;
...
int VIDEO_captureTskRunIsifIn()
{
  ...
  for(streamId=0; streamId<gAVSERVER_config.numCaptureStream; streamId++) {
    ...
    VIDEO_dmvaCopyRun(VIDEO_TSK_CAPTURE, streamId, pBufInfo);
...
int VIDEO_captureRszFunc(Uint8 *inVirtAddr, OSA_BufInfo **pOutBufInfo, int *rszBufId, int numStreams)
{
    ...
    for(streamId=0; streamId<numStreams; streamId++) {
      ...
      if(pOutBufInfo[streamId]!=NULL) {
        ...
        VIDEO_dmvaCopyRun(VIDEO_TSK_CAPTURE, streamId, pOutBufInfo[streamId]);
...
int VIDEO_captureTskRunDdrIn()
{
  ...
  if(pRawBufInfo != NULL)
  {
      ...
      pBufInfo = AVSERVER_bufPutFull( VIDEO_TSK_CAPTURE, 0, rawBufId);
	  if(pBufInfo == NULL)
	  {
		OSA_printf(" CAPTURE: Put Full Buf %d failed\n", rawBufId);
	  }

	  pBufInfo = AVSERVER_bufGetEmpty( VIDEO_TSK_CAPTURE, 0, &freeBufId, OSA_TIMEOUT_FOREVER);
	  if(pBufInfo != NULL)
	  {
	      DRV_isifPutOutBuf(freeBufId);
	  }
