...
//#define DUMP_RSZ_FRAMES
//#define DUMP_RSZ_FRAMES_IN

#if defined(DUMP_RSZ_FRAMES) 
    #define MAX_RSZ_FRAME_DUMP_CNT		(10)
    FILE *rszFile;
	Uint32 gRszDumpFrameCnt;
#endif

#if defined(DUMP_RSZ_FRAMES_IN) 
    #define MAX_RSZ_FRAME_DUMP_CNT_IN		(10)
    FILE *rszFileIn;
	Uint32 gRszDumpFrameCntIn;
#endif
...
int VIDEO_resizeTskRun(int streamId)
{
...
#if defined(DUMP_RSZ_FRAMES) || defined(DUMP_RSZ_FRAMES_IN)   
  Uint32 writeSize;
#endif  
  ...
  if(pInBufInfo!=NULL) {
	...
	for(rszStreamId = 0; rszStreamId < gAVSERVER_config.numCaptureStream; rszStreamId += 2) 
	{									
		...
		if(rszInstance < gAVSERVER_config.numCaptureStream)
		{
			pOutBufInfo[rszStreamId + 1] = AVSERVER_bufGetEmpty( VIDEO_TSK_RESIZE, (rszStreamId + 1), &rszBufId[rszStreamId + 1], OSA_TIMEOUT_FOREVER);
			OSA_assert(pOutBufInfo[rszStreamId + 1] != NULL);		
		
			reszPrm.pOut[1] = &reszOutPrm[1];
			reszOutPrm[1].outType     = gAVSERVER_config.captureYuvFormat;
			reszOutPrm[1].flipH       = FALSE;
			reszOutPrm[1].flipV       = FALSE;
			reszOutPrm[1].outVirtAddr = pOutBufInfo[rszStreamId + 1]->virtAddr + VIDEO_BUF_HEADER_SIZE;
			reszOutPrm[1].outPhysAddr = pOutBufInfo[rszStreamId + 1]->physAddr + VIDEO_BUF_HEADER_SIZE;
			reszOutPrm[1].outWidth    = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutWidth;
			reszOutPrm[1].outHeight   = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutHeight;
			reszOutPrm[1].outOffsetH  = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutOffsetH;
			reszOutPrm[1].outOffsetV  = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutOffsetV;
...
#ifdef DUMP_RSZ_FRAMES_IN
		if(rszFileIn != NULL)
		{
			if(gRszDumpFrameCntIn < MAX_RSZ_FRAME_DUMP_CNT_IN)
			{
				writeSize = (reszPrm.inOffsetH * reszPrm.inOffsetV * 2);

				if(fwrite((reszPrm.inVirtAddr),1,writeSize,rszFileIn) != writeSize)
				{
					printf("RSZ >> Error writing to input file\n");
				}
			}
			else if(gRszDumpFrameCntIn == MAX_RSZ_FRAME_DUMP_CNT_IN)
				 {
					 fclose(rszFileIn);
				 }
...
#ifdef DUMP_RSZ_FRAMES
		if(rszFile != NULL)
		{
			if(gRszDumpFrameCnt < MAX_RSZ_FRAME_DUMP_CNT)
			{
				writeSize = (reszOutPrm[0].outWidth * reszOutPrm[0].outHeight * 3)/2;

				if(fwrite((reszOutPrm[0].outVirtAddr),1,writeSize,rszFile) != writeSize)
				{
					printf("RSZ >> Error writing to input file\n");
				}
			}
			else if(gRszDumpFrameCnt == MAX_RSZ_FRAME_DUMP_CNT)
				 {
					 fclose(rszFile);
				 }
        ...
        VIDEO_dmvaCopyRun(VIDEO_TSK_RESIZE, rszStreamId, pOutBufInfo[rszStreamId]);
		
		if(reszPrm.pOut[1] != NULL)
		{
			pOutBufHeader = (VIDEO_BufHeader*)pOutBufInfo[rszStreamId + 1]->virtAddr;
			
			pOutBufHeader->timestamp = pInBufHeader->timestamp;
			pOutBufHeader->frameNum  = pInBufHeader->frameNum;
			pOutBufHeader->startX    = 0;
			pOutBufHeader->startY    = 0;
			pOutBufHeader->width     = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutWidth;
			pOutBufHeader->height    = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutHeight;
			pOutBufHeader->offsetH   = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutOffsetH;
			pOutBufHeader->offsetV   = gVIDEO_ctrl.captureStream[rszStreamId + 1].captureOutOffsetV;
			
			AVSERVER_bufPutFull( VIDEO_TSK_RESIZE, (rszStreamId + 1), rszBufId[rszStreamId + 1]);
			VIDEO_displayCopyRun(VIDEO_TSK_RESIZE, (rszStreamId + 1), pOutBufInfo[rszStreamId + 1]);
			VIDEO_swosdRun(VIDEO_TSK_RESIZE, (rszStreamId + 1), pOutBufInfo[rszStreamId + 1]);		
        	VIDEO_dmvaCopyRun(VIDEO_TSK_RESIZE, (rszStreamId + 1), pOutBufInfo[rszStreamId + 1]);
		}	
	}
...
#endif // #ifdef YUV_MODE_INTERLACED
  }
...
int VIDEO_resizeCreate()
{
  ...
  if(status!=OSA_SOK) {
    ...
    rszFile = fopen("rsz.yuv","wb");
    if(rszFile == NULL)
    {
        printf("RSZ >> Error opening input file\n");
    }
#endif  
  
#ifdef DUMP_RSZ_FRAMES_IN
    gRszDumpFrameCntIn = 0;

    rszFileIn = fopen("rszIn.yuv","wb");
    if(rszFileIn == NULL)
    {
        printf("RSZ >> Error opening input file\n");
    }
