...
int VIDEO_vnfTskCreate(Int32 sysRegBase)
{
  ...
  {
#ifdef TNF3_MODE_ON
    createPrm.width           = OSA_align(gAVSERVER_config.captureConfig[streamId].width+ALG_VNF_PIXEL_PAD,32);
#else
	...
	if(  gAVSERVER_config.captureConfig[streamId].snfEnable
	  ...
	  createPrm.mode            = DEFAULT_VNF_MODE_ONLY_TNF;
	}
	else if(gAVSERVER_config.captureConfig[streamId].snfEnable ) {
	  createPrm.mode            = DEFAULT_VNF_MODE_ONLY_SNF;
	}
	else {
	  createPrm.mode            = DEFAULT_VNF_MODE_OFF;
	}
...
{
  int status=OSA_EFAIL, inBufId = 0, outBufId = 0;
  ...
  if(pInBufInfo!=NULL) {
    ...
    if(pOutBufInfo!=NULL) {
      ...
      vnfPrm.pVnfParams   = NULL;
        ...
        OSA_printf(" VNF: Stream %d InBuf %d OutBuf %d (%d,%d)\n",
    	 ...
    	 && gAVSERVER_config.captureConfig[streamId].tnfEnable
    	  ...
    	  vnfPrm.mode            = DEFAULT_VNF_MODE;
      }
      else if(gAVSERVER_config.captureConfig[streamId].tnfEnable ) {
    	  vnfPrm.mode            = DEFAULT_VNF_MODE_ONLY_TNF;
      }
      else if(gAVSERVER_config.captureConfig[streamId].snfEnable ) {
        vnfPrm.mode            = DEFAULT_VNF_MODE_ONLY_SNF;
      }
      else {
        vnfPrm.mode            = DEFAULT_VNF_MODE_OFF;
      }

      vnfPrm.strength          = gAVSERVER_config.captureConfig[streamId].vnfStrength;
      vnfPrm.strUpdate         = gAVSERVER_config.captureConfig[streamId].vnfPrmUpdate;

	  gAVSERVER_config.captureConfig[streamId].vnfPrmUpdate = (vnfPrm.strength==TNF_AUTO)? TRUE:FALSE;

      if(   gAVSERVER_config.captureConfig[streamId].tnfEnable
         || gAVSERVER_config.captureConfig[streamId].snfEnable
         ) {

		if(vnfPrm.mode==ALG_VNF_MODE_ONLY_TNF) {
			vnfPrm.inStartX = 16;
			vnfPrm.inStartY = 9;
		}
		if(vnfPrm.mode==ALG_VNF_MODE_ONLY_TNF3) {
			vnfPrm.inStartX = 16;
			vnfPrm.inStartY = 18;
		}
		...
		pOutBufHeader->offsetH = vnfStatus.outPitch; //New
        if(vnfPrm.mode==ALG_VNF_MODE_ONLY_TNF)
        {
      ...
      VIDEO_dmvaCopyRun(VIDEO_TSK_VNF, streamId, pOutBufInfo);
