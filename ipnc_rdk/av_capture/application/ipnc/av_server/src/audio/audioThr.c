...
int AUDIO_GetSampleRate( void )
{
	...
	fd_proc = open("/sys/kernel/debug/davinci_clocks", O_RDONLY);
	if( fd_proc < 0 )
	{
	...
	while( pStr != NULL )
	{
		...
		if( strncmp(TmpBuf, "voice_codec", sizeof("voice_codec")) == 0 )
		{
...
int AUDIOPLAY_audioTskRun(Bool *pDone)
{
	...
	switch(rbuf.msg)
	{
	    case AUD_STREAM_NEWFRAME:
			...
			{
				rc = DRV_audioplayset_param(&gAUDIOPLAY_ctrl.audioHndl, &Audioplayconfig);
				if (rc != OSA_SOK)
				{
					snd_msg(snd_msg_qid);
					return rc;
