...
#define VIDEO_TSK_DEI           (0x0004)
#define VIDEO_TSK_VNF           (0x0008)
#define VIDEO_TSK_RESIZE        (0x0010)
#define VIDEO_TSK_ENCODE        (0x0020)
#define VIDEO_TSK_ENCRYPT       (0x0040)
#define VIDEO_TSK_STREAM        (0x0080)
#define VIDEO_TSK_DMVAL         (0x0100) 

#define VIDEO_BUF_HDR_BASE_SIZE (128)

#define VNF_PADDING				(32)
#define VNF_MAX_WIDTH			(1920)
#ifdef TNF3_MODE_ON
#define ADDI_HEIGHT   			(7)
#else
#define ADDI_HEIGHT   			(0)
#endif

#define ADDI_BUFFERING 			((VNF_MAX_WIDTH + VNF_PADDING)*ADDI_HEIGHT)
#define VIDEO_BUF_HEADER_SIZE   (VIDEO_BUF_HDR_BASE_SIZE + ADDI_BUFFERING)
#define VIDEO_BUF_METADATA_SIZE (128)
...
typedef struct {
  ...
  OSA_BufHndl bufDmvaIn;
  OSA_BufCreate bufDmvaInCreatePrm;
  OSA_MutexHndl statusLock;
  OSA_MutexHndl snapShotLock; 
  Bool dmvaError;
  DMVALhandle algDmvalHndl;
  DMVALzones zones;
...
typedef struct {
  ...
  VIDEO_DmvaAppStream    dmvaAppStream;
