...
int DMA_copyFillCreate(DMA_OpenClosePrm *prm)
{
  ...
  pObj->copyFillObj->chId = edma_alloc_channel(EDMA_CHANNEL_ANY, DMA_copyFillCallback,pObj->copyFillObj, DMA_NORMAL_EVENT_QUEUE);
  if (pObj->copyFillObj->chId < 0)
  {
  ...
  if(pObj->copyFillObj->chId >= DMA_DEV_MAX_CH) {
    ...
    edma_clean_channel( pObj->copyFillObj->chId );
    edma_free_slot( pObj->copyFillObj->chId );
