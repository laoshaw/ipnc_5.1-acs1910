...
int DRV_imageTuneInit()
{
  ...
  pTnf3Cfg  = ImageTune_imgsGetTnf3Config(0);
  ...
  if (pTnf3Cfg != NULL) 
    gDRV_imageTuneObj.imgsDefaultPrm.pTnf3Prm = &pTnf3Cfg;
...
int DRV_imageTuneLoadParams(int mode, int paramSetId)
{
  ...
  if(retVal==OSA_SOK) {
    ...
    pTnf3Cfg = ImageTune_imgsGetTnf3Config(mode);
