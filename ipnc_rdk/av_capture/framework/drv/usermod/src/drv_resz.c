...
int DRV_reszRun(DRV_ReszRunPrm *pPrm)
{
  ...
  int status=OSA_EFAIL, i, startX, startY;
  CSL_RszOutConfig *pOutConfig;
  ...
  for(i=0; i<CSL_RSZ_CH_MAX; i++)
  {
    ...
    pOutConfig->inStartY               = startY;
      ...
      pOutConfig->yuv420COutEnable       = FALSE;
        ...
        // enabling down-scaler

		totalReszRatioHorzFloat = (float)((Uint32)prm.inWidth)/prm.pOut[i]->outWidth;
		totalReszRatioVertFloat = (float)((Uint32)prm.inHeight)/prm.pOut[i]->outHeight;
		...
		if(totalReszRatioHorzFloat >= 4.0)
		{
			...
			// Horz resize
			cnt                = 0;
			totalReszRatioHorz = (Uint32)prm.inWidth/prm.pOut[i]->outWidth;
			...
			// Vert resize
			cnt                = 0;
			totalReszRatioVert = (Uint32)prm.inHeight/prm.pOut[i]->outHeight;
