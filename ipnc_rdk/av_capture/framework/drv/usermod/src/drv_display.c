...
DRV_DisplayObj *gPDisplayObj = NULL;

static void DRV_displaySetObj(DRV_DisplayObj *pObj)
{
	gPDisplayObj = pObj;
}

static DRV_DisplayObj *DRV_displayGetObj()
{
	return gPDisplayObj;
}
...
int DRV_displaySetMode(int mode)
{
	...
	memset(&output,0x00,sizeof(struct v4l2_output));

	switch(mode)
	{
		case DRV_DISPLAY_MODE_NTSC:
			output.index = 0;					// Composite
			standard_id  = V4L2_STD_NTSC;
			expandH		 = pObj->config.expandH;
			expandV      = 0;
			break;
		...
		case DRV_DISPLAY_MODE_1080I:
			...
			reqpreset.preset = V4L2_DV_1080I30;
			break;

		case DRV_DISPLAY_MODE_720P:
			output.index     = 1;				// Component
			reqpreset.preset = V4L2_DV_720P30;
			break;
	}

	status = ioctl(pObj->fd, VIDIOC_S_OUTPUT, &output.index);
	if(status < 0) {
		OSA_ERROR("VIDIOC_S_OUTPUT for %d\n",output.index);
	}

	if(output.index == 0)
	{
		// Component
		status = ioctl(pObj->fd, VIDIOC_S_STD, &standard_id);
		if(status < 0) {
			OSA_ERROR("VIDIOC_S_STD for %d\n", (int)standard_id);
		}
	}
	else if(output.index == 1)
		 {
			 // Composite
			 status = ioctl(pObj->fd, VIDIOC_S_DV_PRESET, &reqpreset);
			 if(status < 0) {
				 OSA_ERROR("VIDIOC_S_DV_PRESET for %d\n",reqpreset.preset);
			 }
		 }
	...
	status = ioctl(pObj->fd, VIDIOC_CROPCAP, &cropcap);
	if(status) {
	...
	status = ioctl(pObj->fd, VIDIOC_S_CROP, &crop);
	if(status < 0) {
