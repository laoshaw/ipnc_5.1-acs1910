...
int DRV_ipipeRszSetParams()
{
  ...
  int totalReszRatioHorz,totalReszRatioVert,cnt;
  float totalReszRatioHorzFloat,totalReszRatioVertFloat;
  ...
  for(i=0; i<CSL_RSZ_CH_MAX; i++)
  {
    ...
    pOutConfig->dmaInterval 		   = gDRV_ipipeObj.config.rszOutConfig[i].dmaInterval;
	...
	totalReszRatioHorzFloat = (float)((Uint32)gDRV_ipipeObj.ipipeInFrameConfig.inWidth)/pOutConfig->outWidth;
	totalReszRatioVertFloat = (float)((Uint32)gDRV_ipipeObj.ipipeInFrameConfig.inHeight)/pOutConfig->outHeight;

	if(totalReszRatioHorzFloat >= 4.0)
    {
		// Enable Down scaler
		pOutConfig->downScaleModeEnable = TRUE;

		// Horz resize
		cnt                = 0;
		totalReszRatioHorz = (Uint32)gDRV_ipipeObj.ipipeInFrameConfig.inWidth/pOutConfig->outWidth;
        while(totalReszRatioHorz)
        {
		    totalReszRatioHorz = totalReszRatioHorz >> 1;
		    cnt ++;
		}
		...
		pOutConfig->horzReszRatio          = (int)((totalReszRatioHorzFloat * 256)/(1 << cnt));
		pOutConfig->downScaleModeAvgSizeH  = (cnt - 1);

        // Vert resize
		cnt                = 0;
		totalReszRatioVert = (Uint32)gDRV_ipipeObj.ipipeInFrameConfig.inHeight/pOutConfig->outHeight;
        while(totalReszRatioVert)
        {
		    totalReszRatioVert = totalReszRatioVert >> 1;
		    cnt ++;
		}
		...
		pOutConfig->vertReszRatio          = (int)((totalReszRatioVertFloat * 256)/(1 << cnt));
		pOutConfig->downScaleModeAvgSizeV  = (cnt - 1);
	}
...
int DRV_ipipeSetGamma (Uint32* table)
{
    int status = OSA_SOK;
    CSL_IpipeGammaConfig data = {
    ...
    status = CSL_ipipeSetGammaConfig(&gCSL_ipipeHndl, &data);
    if(status != OSA_SOK){
        return OSA_EFAIL;
    }
