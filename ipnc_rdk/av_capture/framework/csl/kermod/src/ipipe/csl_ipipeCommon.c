...
#else
  for(i=0; i<data->lutNumEntries && i<CSL_IPIPE_DPC_LUT_MAX_ENTRIES/2; i++) {
    *tableMemAddr[0]++ = data->lutAddr[i];
  }
  for(; i<data->lutNumEntries && i<CSL_IPIPE_DPC_LUT_MAX_ENTRIES; i++) {
    *tableMemAddr[1]++ = data->lutAddr[i];
  }
#endif
  }
...
//#deinfe GAMMA_TABLE_DEBUG
#ifdef GAMMA_TABLE_DEBUG
  if(data->tableR) {

    tableMemAddr = (Uint32*)CSL_IPIPE_GAMMA_R_TBL_3_ADDR;

    for(i=0; i<tableSize; i++) {
      if(*tableMemAddr++ != data->tableR[i])
      	printk("Gamma Table R not matching %d\n", i);
    }
  }
  ...
  if(data->tableG) {
    ...
    for(i=0; i<tableSize; i++) {
      if(*tableMemAddr++ != data->tableG[i])
      	printk("Gamma Table G not matching %d\n", i);
    }
  }
  ...
  if(data->tableB) {
    ...
    for(i=0; i<tableSize; i++) {
      if(*tableMemAddr++ != data->tableB[i])
      	printk("Gamma Table B not matching %d\n", i);
    }
  }
#endif
