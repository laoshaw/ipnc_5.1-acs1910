...
static Uint8 * lastAfAddr= NULL;
static Uint16 afNumWinH= 0;
static Uint16 afNumWinV= 0;
...
int ALG_afRun(ALG_AfRunPrm * prm, int *focus_value)
{
  ...
  afNumWinV= af_pax_vt_cnt = prm->pH3aInfo->afNumWinV;
  afNumWinH= af_pax_hz_cnt = prm->pH3aInfo->afNumWinH;

    lastAfAddr= curAfAddr = (Uint8* ) prm->h3aDataVirtAddr;
...
int ALG_afGetLastInfo(Uint8 **ptr2LastAfAddr, Uint16 *ptr2numWinH, Uint16 *ptr2numWinV){
    *ptr2LastAfAddr= lastAfAddr;
    *ptr2numWinH= afNumWinH;
    *ptr2numWinV= afNumWinV;
