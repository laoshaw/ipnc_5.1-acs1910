...
//#define OLD_FRAME_CHECK
#define NUM_LASTLINES	(0)
...
void *ALG_vidEncCreate(ALG_VidEncCreate *create)
{
  ...
  else if(create->codec==ALG_VID_CODEC_H264)
  {
	  ...
	  /* Switch OFF packet size at interface for better performance and put it to fixed 100% */
	  pObj->params_h264.sliceMode = (pObj->params.encodingPreset==XDM_HIGH_SPEED)?0:3;
  }
...
int ALG_vidEncSetFramePitch(void *hndl, int framePitch)
{
  ...
  pObj->createPrm.offsetH = framePitch;
  status = ALG_vidEncSetDynamicParams(pObj);
...
int ALG_vidEncRun(void *hndl, ALG_VidEncRunPrm *prm, ALG_VidEncRunStatus *runStatus)
{
  ...
  /////////////////////////////////////////////////////////////////////////////////////
  // Need to update the captureWidth of Codec if the framePitch has changed.
  // This can happen not only at the start of encoding, can also occur
  // say when use rswitches from TNF only mode to SNF+TNF where no teardown happens.
  /////////////////////////////////////////////////////////////////////////////////////
  if(pObj->createPrm.offsetH != prm->framePitch)
  {
     ALG_vidEncSetFramePitch(pObj, prm->framePitch);
  }
  ...
  if(prm->snapEnable) {
  	if(prm->snapMode==SNAPSHOT_YUV) {
		...
		OSA_mutexLock(&vicp_imcop_lock);
  		SnapshotFrameData(prm->snapFilename, (Uint8 *)inBufDesc.bufDesc[0].buf, (Uint8 *)inBufDesc.bufDesc[1].buf,
  			inBufDesc.frameWidth, inBufDesc.frameHeight, inBufDesc.framePitch);
  		OSA_mutexUnlock(&vicp_imcop_lock);
	}
  }
...
#ifdef OLD_FRAME_CHECK
	{
		if(pObj->createPrm.offsetH>=1920)
		{
			volatile unsigned char *ptr1 = &prm->inAddr[(pObj->createPrm.offsetH*(inBufDesc.frameHeight-NUM_LASTLINES))-16];
			volatile unsigned int *ptr;

			ptr = (unsigned int *)ptr1;

			OSA_cmemCacheInv(ptr, 64);

			if(*ptr == (unsigned int)(0xDEADBEEF)) {
				printf("FOUND THE OLD_FRAME_CHECK CONDITION\n");
			}
			else {
			}
		}
...
int ALG_vidEncTestMain(int argc, char **argv)
{
  ...
  for(i=0; i<numFrames; i++) {
	...
	runPrm.framePitch     = createPrm.offsetH;
