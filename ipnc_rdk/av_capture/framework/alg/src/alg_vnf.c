...
void   *ALG_vnfCreate(ALG_VnfCreate * create)
{
  ...
  pObj->extDynParams.videncDynamicParams.inputHeight = create->height; //pObj->params.maxHeight;
  pObj->extDynParams.videncDynamicParams.inputWidth  = create->width; //pObj->params.maxWidth;
  ...
  switch(create->mode) {
    ...
    case ALG_VNF_MODE_KTNF:
        ...
        break;
    case ALG_VNF_MODE_TNF3:
        pObj->extParams.EnableNSF = IKTNF_TNF3;
        break;
    case ALG_VNF_MODE_ONLY_TNF3:
        pObj->extParams.EnableNSF = IKTNF_TNF3_ONLY;
        break;
  }
  ...
  //TNF3 Parameters
  pObj->extParams.tnf3TS 			= 7;
  pObj->extParams.unDiffScaleFactor = 2;
  pObj->extParams.unMotionThreshold = 12;
  pObj->extParams.unStrengthOfTNF3 	= 9;
  ...
  /*Set tnf3 structure*/
  IMAGE_TUNE_SetTnf3Params((ALG_vnfParams*) &pObj->extParams.tnf3TS);
...
int ALG_vnfRun(void *hndl, ALG_VnfRunPrm * prm, ALG_VnfStatus * runStatus)
{
  ...
  if(prm->pVnfParams!=NULL) {
    ...
    IMAGE_TUNE_GetTnf3Params((ALG_tnf3Params*) &pObj->extDynParams.tnf3TS );
  }

  if(prm->strUpdate==TRUE)
  {
	  if(prm->strength==TNF_AUTO)
	  {
		if(gOutAWBData.sensorGain < 2000)
			prm->strength = TNF_LOW;
		else if(gOutAWBData.sensorGain < 32000)
			prm->strength = TNF_MED;
		else
			prm->strength = TNF_HIGH;
	  }

	  switch(prm->strength)
	  {
		case TNF_LOW:
			pObj->extDynParams.TNF_A0 = 75;
			pObj->extDynParams.TNF_TM = 8;

			pObj->extDynParams.tnf3TS 			 = 7;
			pObj->extDynParams.unDiffScaleFactor = 2;
			pObj->extDynParams.unMotionThreshold = 5;
			pObj->extDynParams.unStrengthOfTNF3  = 3;
		break;
		default:
		case TNF_MED:
			pObj->extDynParams.TNF_A0 = 50;
			pObj->extDynParams.TNF_TM = 12;

			pObj->extDynParams.tnf3TS 			 = 8;
			pObj->extDynParams.unDiffScaleFactor = 2;
			pObj->extDynParams.unMotionThreshold = 8;
			pObj->extDynParams.unStrengthOfTNF3  = 8;
		break;
		case TNF_HIGH:
			pObj->extDynParams.TNF_A0 = 25;
			pObj->extDynParams.TNF_TM = 18;

			pObj->extDynParams.tnf3TS 			 = 9;
			pObj->extDynParams.unDiffScaleFactor = 2;
			pObj->extDynParams.unMotionThreshold = 15;
			pObj->extDynParams.unStrengthOfTNF3  = 10;
		break;
	  }
  ...
  switch(prm->mode) {
    ...
    case ALG_VNF_MODE_KTNF:
      ...
      break;
    case ALG_VNF_MODE_TNF3:
      pObj->extDynParams.EnableNSF = IKTNF_TNF3;
      break;
    case ALG_VNF_MODE_ONLY_TNF3:
      pObj->extDynParams.EnableNSF = IKTNF_TNF3_ONLY;
      break;
  }
  ...
  status = VIDENC1_control((VIDENC1_Handle)pObj->hEncode, XDM_SETPARAMS, (IVIDENC1_DynamicParams *)&pObj->extDynParams, (IVIDENC1_Status *)&VNF_status);
  ...
  if((prm->mode==ALG_VNF_MODE_TNF3) || (prm->mode==ALG_VNF_MODE_ONLY_TNF3))
  {
	  bufferSize = inOffset = outOffset = VNF_status.outPitch*pObj->createPrm.offsetV;
	 ...
	 /* Output buffers */
	  ...
	  //outbufs[1] = (XDAS_Int8*)(prm->outAddr + outOffset - 4*VNF_status.outPitch);
  ...
  runStatus->outPitch  = outArgs.outPitch;
...
int ALG_vnfTestMain(int argc, char **argv)
{
  ...
  createPrm.mode = DEFAULT_VNF_MODE;
...
ALG_tnf3Params* ImageTune_imgsGetTnf3Config(mode)
{
