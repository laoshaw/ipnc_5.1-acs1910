...
/* DMVA ALARM EVENT */
/**
* @brief DMVA ALARM EVENT
*
* param Serial [I ] Serial Number of DMVA Currently it is 0
*/
void SendDMVAAlarmEventDetect(int EvtType,char *fileName)
{
	...
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = 0;
	msgbuf.event = ALARM_DMVAEVENT_TRIP_ZONE;
	msgbuf.serial = EvtType;
	msgbuf.alarmaudio_duration = (gProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;             
	msgbuf.alarmaudio_enable   = strlen(fileName);
	ShareMemWrite(msgbuf.alarmaudio_duration,fileName,msgbuf.alarmaudio_enable);            
	msgsnd(qid, &msgbuf, sizeof(msgbuf)-sizeof(long), 0);/*send msg1*/
}
