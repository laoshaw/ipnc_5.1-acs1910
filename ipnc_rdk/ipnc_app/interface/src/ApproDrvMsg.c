...
int SetVNFParamMsg(unsigned char stream, VnfParam* vnfParam)
{
    MSG_BUF msgbuf;
    unsigned char* ptr;
	...
	if (stream == 0)
		msgbuf.cmd = MSG_CMD_SET_VNF_PARAM1;
	else if (stream == 1)
		msgbuf.cmd = MSG_CMD_SET_VNF_PARAM2;
	else if (stream == 2)
		msgbuf.cmd = MSG_CMD_SET_VNF_PARAM3;
