...
int faddscopes(Onvif_Scopes_t onvif_scopes_tmp)
{
	...
	while(strlen(oSysInfo->onvif_scopes.type[i]) != 0)
	{
		i++;
	}
	while(strlen(oSysInfo->onvif_scopes.location[j]) != 0)
	{
		j++;
	}
	...
	for(itr = 0;itr < k;itr++)
	{
		memcpy(&oSysInfo->onvif_scopes.type[i], &onvif_scopes_tmp.type_t[itr], sizeof(onvif_scopes_tmp.type_t[itr]));
		i++;
	}
	for(itr = 0;itr < l;itr++)
	{
		memcpy(&oSysInfo->onvif_scopes.location[j], &onvif_scopes_tmp.location_t[itr], sizeof(onvif_scopes_tmp.location_t[itr]));
		j++;
	}
...
int fdelscopes(Onvif_Scopes_t onvif_scopes_tmp)
{
	...
	int num_tp = 0;
	int num_loc = 0;
	while(strstr(oSysInfo->onvif_scopes.type[num_tp], ONVIF_SCOPE) != NULL)
	{
		TYPE_MAX++;
		num_tp++;
	}
	while(strstr(oSysInfo->onvif_scopes.location[num_loc], ONVIF_SCOPE) != NULL)
	{
		LOC_MAX++;
		num_loc++;
	}
	...
	while(strstr(onvif_scopes_tmp.type_t[i], ONVIF_SCOPE) != NULL)
	{
		while(strstr(oSysInfo->onvif_scopes.type[num_tp], ONVIF_SCOPE) != NULL)
		{
			if(strcmp(onvif_scopes_tmp.type_t[i], oSysInfo->onvif_scopes.type[num_tp]) == 0)
			{
				break;
			}
			num_tp++;
		...
		{
			memcpy(&oSysInfo->onvif_scopes.type[num_tp], &oSysInfo->onvif_scopes.type[TYPE_MAX-1], sizeof(oSysInfo->onvif_scopes.type[num_tp]));
			memset(&oSysInfo->onvif_scopes.type[TYPE_MAX-1], 0, sizeof(oSysInfo->onvif_scopes.type[TYPE_MAX-1]));
		}
		i++;
	}
	while(strstr(onvif_scopes_tmp.location_t[j], ONVIF_SCOPE) != NULL)
	{
		while(strstr(oSysInfo->onvif_scopes.location[num_loc], ONVIF_SCOPE) != NULL)
		{
			if(strcmp(onvif_scopes_tmp.location_t[j], oSysInfo->onvif_scopes.location[num_loc]) == 0)
			{
				break;
			}
			num_loc++;
		...
		{
			memcpy(&oSysInfo->onvif_scopes.location[num_loc], &oSysInfo->onvif_scopes.location[LOC_MAX-1], sizeof(oSysInfo->onvif_scopes.location[num_loc]));
			memset(&oSysInfo->onvif_scopes.location[LOC_MAX-1], 0, sizeof(oSysInfo->onvif_scopes.location[LOC_MAX-1]));
		}
		j++;
	}
}
