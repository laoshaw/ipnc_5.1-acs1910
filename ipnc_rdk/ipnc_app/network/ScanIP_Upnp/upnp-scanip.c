...
*/
	...
	if( ControlSystemData(SFIELD_GET_MAC, (void *)&mac, sizeof(mac)) < 0 ){
		...
		sprintf(macadress,"_unknown_mac_add_");
	}
	else{
		sprintf(macbuff,"%02x",mac[0]);
		strcat(macadress,macbuff);
		for(cnt =1;cnt < 6;cnt++)
	{
			sprintf(macbuff,":%02x",mac[cnt]);
			strcat(macadress,macbuff);
	}
	}
	while(!feof(fptr))
	{
		if(fread(tmpBuff,1,MAX_FILE_SIZE,fptr)==0)
			break;
	}
	
		//	Find "uuid" offset
	offset = strstr(tmpBuff , udn );
	
	fseek(fptr,offset - tmpBuff,SEEK_SET);
#if 0
	   unsigned long udp=0;
	   randomize();
	   udp=rand()%100000000;	   	
...
#endif
	sprintf(tmpString,"uuid:Upnp-TVEmulator-%s</UDN>\n",macadress);
	printf("<UDN>%s\n",tmpString);
       tmpptr = tmpString;
		//	Write unique number to tag <UDN>
	for (cnt=0;cnt<strlen(tmpString);cnt++)
	{
		fputc(*tmpptr, fptr);
		tmpptr++;
	};	
