...
int retrieve_system_deviceinfo(request *req, node_t *node)
{
	...
	switch(pSysInfo->lan_config.chipConfig)
	{
		...
		case CHIP_DM368E:
			...
			break;
		case CHIP_DM369:
			sprintf(DeviceInfo.firmware_version,"%s", DM369_APP_VERSION);
			break;
		case CHIP_DMVA1:
			sprintf(DeviceInfo.firmware_version,"%s", DMVA1_APP_VERSION);
			break;
		case CHIP_DMVA2:
			sprintf(DeviceInfo.firmware_version,"%s", DMVA2_APP_VERSION);
			break;
	}
...
int create_security_aaa_users(request *req, node_t *node)
{
	...
	while(tag) {
		...
		memset(&account,0x00,sizeof(add_user_t));
...
int delete_security_aaa_users(request *req, node_t *node)
{
	...
	for(i = 0; i < ACOUNT_NUM; i++) {
		...
		strLen = strlen(User.userName);
		if(strLen > 0)
		{
		   User.userName[strLen] = '\0';
		   ControlSystemData(SFIELD_DEL_USER, (void *)User.userName, strLen + 1);
		}
	...
	while(tag) {
		if(strcmp(tag->name, TAG_id) == 0) {
			...
			required |= xml_required_field(1);
		        ...
		        (strstr(tag->name, TAG_userName) != NULL)) {
			if(tag->value != NULL)
				strcpy(User.userName, tag->value);
			else
				User.userName[0] = '\0';
...
int retrieve_custom_camera_channels_id(request *req, node_t *node)
{
	...
	sprintf(CameraSetting.dynRange,"%d",pSysInfo->advanceMode.dynRange);
	sprintf(CameraSetting.vsEnabled,"%d",pSysInfo->advanceMode.vsEnable);
	sprintf(CameraSetting.ldcEnabled,"%d",pSysInfo->advanceMode.ldcEnable);
	...
	{
		if(pSysInfo->advanceMode.vnfMode!=FFLAG_TNF)
	   		sprintf(CameraSetting.snfltCtrl,"ON");
	   	else
	   		sprintf(CameraSetting.snfltCtrl,"OFF");

		if(pSysInfo->advanceMode.vnfMode!=FFLAG_SNF)
			sprintf(CameraSetting.tnfltCtrl,"%d",1);
		else
			sprintf(CameraSetting.tnfltCtrl,"%d",0);
 	}
	else {
		sprintf(CameraSetting.snfltCtrl,"OFF");
		sprintf(CameraSetting.tnfltCtrl,"%d",0);
