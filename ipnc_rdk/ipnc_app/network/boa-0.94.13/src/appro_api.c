...
void add_user(request *req, COMMAND_ARGUMENT *argm)
{
	...
	do {
	    ...
	    if((strlen(acount.user_id) > USER_LEN) || (strlen(acount.user_id) < MIN_USER_LEN))
	        break;

	    if((strlen(acount.password) > PASSWORD_LEN) || (strlen(acount.password) < MIN_PASSWORD_LEN))
	    	break;
...
void set_dynrangestrength(request *req, COMMAND_ARGUMENT *argm)
{
	...
	do {
		ControlSystemData(SFIELD_SET_DYNRANGESTR, (void *)&value, sizeof(value));
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void restart_ipcam(request *req, COMMAND_ARGUMENT *argm)
{
	...
	SysInfo* pSysInfo = GetSysInfo();
	__u8 value,nAlarmAudioPlay;


    nAlarmAudioPlay = pSysInfo->lan_config.nAlarmAudioPlay;
    pSysInfo->lan_config.nAlarmAudioPlay = 0;

    // umount the SD card if mounted
    if(pSysInfo->sdcard_config.sdinsert == 0x3) // SD card is inserted and mounted
    {
		if(pSysInfo->mmcqdPid != 0)
		{
		    sprintf(cmd,"renice -5 %d\n",pSysInfo->mmcqdPid);
			system(cmd);
			fprintf(stderr,"\n*** priority of mmcqd(%d) is changed to %d\n",pSysInfo->mmcqdPid,getpriority(0,pSysInfo->mmcqdPid));
		}

        value = 1;
		if (ControlSystemData(SFIELD_SD_UNMOUNT, (void *)&value, sizeof(value)) < 0)
		{
		    fprintf(stderr,"\n*** SD card unmount failed\n");
		}
	}
	...
	sprintf(cmd, "killall -2 av_server.out \n");
	system(cmd);
	sleep(5);
	...
	*pPTCMD 		= 1;
    ...
    pSysInfo->lan_config.nAlarmAudioPlay = nAlarmAudioPlay;
    SetSysInfoImmediately();

	fprintf(stderr,"NOTE : Please ignore the filesystem error message. This is not a harmful message for ipnc\n\n");
	system ("chmod 777 /tmp/shutdown");
	sprintf(cmd, "/tmp/shutdown -r now\n");
	system(cmd);
