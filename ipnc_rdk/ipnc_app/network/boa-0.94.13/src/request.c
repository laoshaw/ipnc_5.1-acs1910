...
static int sts = 0;
int midhval=0;//mid+++++++++++++++++++
int midhnewsock=0;//mid+++++++++++++++++++
volatile int udp_status = 0;
struct SOCKADDR onvif_remote_addr;
/* Onvif */
//#define PRINT_REQ
...
//======================Onvif=========================//
	midhnewsock = fd;//mid++++++++++
	onvif_remote_addr = remote_addr;//mid++++++++++++
//======================Onvif=========================//
 ...
 * Name: lookupservices
 *
 * Description:  The function parses the ONVIF URI and sends back the appropriate lookup index
   @Input: service - Contains the incoming ONVIF request URI
   @Output: lookupindex
*/
...
/*
 * Name: isOnvif
 * Description:  This function checks whether the incoming URI is valid ONVIF request
 * @Input: current_client_stream - The incoming HTTP request
 * @Output: service_uri - The URI obtained from the incoming packet
 *    lookupindex - The index identifying the type of service
*/
int isOnvif(char *current_client_stream , char *service_uri, int *lookupindex)
{
	/* Store segments of the Onvif POST */
	char segments[3][100];
	char post_req[80];
 ...
 */
void process_requests(int server_s,int server_udp,struct soap *soap)
{
    int retval = 0;
...
/*         printf("SSL request received!!\n");*/
	...
	time(&current_time);
	/* Onvif */
	/* 'isOnvif' function checks whether the incoming request is an ONVIF POST request or not */
	...
	if(OnvifEN == 1)
 	{
		...
		strcpy(ONVIF_BUFFER, current->client_stream);
#if ONVIF_LOG
		fprintf(stderr, "[onvif] received from IP[%s]:%d.%d.%d.%d \n (%s)\n ",soap->tag,
			((soap->ip >> 24) & 0xff), ((soap->ip >> 16) & 0xff),
			//((soap->ip >>  8) & 0xff), ( soap->ip        & 0xff),  ONVIF_BUFFER);
			((soap->ip >>  8) & 0xff), ( soap->ip        & 0xff),  current->client_stream);
#endif
		...
		/* Passing the Service */
		int errorCode = 0;
		if (errorCode = soap_serve_request(soap, service_uri, lookupindex)) 
		{
		...
		/* Deallocate soap structure variables */
...
int process_header_end(request * req,node_t * node)
{
    ...
    if (req->http_uri) {
     ...
     if (req->psia_uri) {                      /*PSIA Handler Initialization*/
	    int ret,content_length = 0,bytesRead,bytes2Read;
		int bytes_to_write;
		//char inbuf[RDBFLEN];
		char *buffer;
