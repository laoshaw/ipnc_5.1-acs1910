...
#endif /*SERVER_SSL*/
/* Boa UDP Socket */
int server_udp;                 /* kept out for bye/hello/probe */
...
static int create_server_socket_udp_hello(void);
...
/**
 * @brief Hello thread Handler 
 * @This thread calls the Hello function based on the discovery mode set in the camera
    Camera is discoverable if the discovery mode is set to 0
*/
...
int main(int argc, char **argv)
{
	...
	/* Socket descriptor created for UDP */
	...
	/* Start-ONVIF */
	/* ONVIF: Hello thread spawned using UDP socket descriptor */	
	if (pthread_create(&hello_thread,NULL,hello_thr,server_udp) != 0)
	{
		ONVIF_DBG("Error:Hello thread creation failed");
	}
	
	/* ONVIF: Signal Handlers for Bye Message before reboot */
        signal(SIGTERM, ByeHandler);   
        signal(SIGUSR1, ByeHandler);   

	/* Soap instance for handling ONVIF TCP requests */	
	soap=soap_new();
	soap_init(soap);
	...
	soap->bind_flags = 1;
	/* End-ONVIF */
	select_loop(server_s,server_udp,soap);
	return 0;
}
