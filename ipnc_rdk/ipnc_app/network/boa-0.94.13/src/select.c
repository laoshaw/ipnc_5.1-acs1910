...
/**
 * @brief Probe thread Handler 
 * @This thread calls the Probe function based on the discovery mode set in the camera
 *  Camera is discoverable if the discovery mode is set to 0
 *  Handles Unicast and Multicast Probe
*/
...
void select_loop(int server_s,int server_udp,struct soap *soap)
{
    ...
    /* ONVIF: Probe Thread initialization */
    if (pthread_create(&probe_thread, NULL, probe_thr, server_udp) != 0)
    {
    	ONVIF_DBG("Error:Probe thread creation failed");
    }
    while (1) {
        if (sighup_flag)
            sighup_run();
        ...
        process_requests(server_s,server_udp,soap);
