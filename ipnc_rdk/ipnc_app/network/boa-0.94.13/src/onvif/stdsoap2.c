...
#if ONVIF_LOG
    static sendbuf[1024 * 10];
    snprintf(sendbuf, n, "%s", s );
    fprintf(stderr, "[onvif] fsend  %s\n", sendbuf );
#endif
...
{ register int r;
  ...
  { for (;;)
...
#endif /*Onvif Change: The below code was slightly changed to accomodate both UDP and TCP requests*/
      { 
#ifndef WITH_LEAN
        if ((soap->omode & SOAP_IO_UDP))
	...
	r = strlen(ONVIF_BUFFER);
	strcpy(soap->buf,ONVIF_BUFFER);
        } 
#ifdef PALM
...
/*	soap->http_content = "text/html"; 
	...
	soap_end_send(soap);*/ 
	//return SOAP_OK;
	//process_requests(soap->socket);
	//get_request(soap->socket);
...
#endif
    { 
      if(strcmp(soap->http_version,"0.9") == 0 || strcmp(soap->http_version,"1.0") == 0 || strcmp(soap->http_version,"1.1") == 0)
        sprintf(soap->tmpbuf, "HTTP/%s %s", soap->http_version, s);
      else
        sprintf(soap->tmpbuf, "HTTP/%s %s", "1.1" , s);
      if ((err = soap->fposthdr(soap, soap->tmpbuf, NULL)))
        return err;
    }
