...
void GetDynRange(char *dynRange)
{
	...
	if(pSysInfo->advanceMode.dynRange == 2)
	{
		strcpy(dynRange, "Local Enhancement");
	}
	else if(pSysInfo->advanceMode.dynRange == 1)
	{
...
void GetDMVAAlgo(char *dmvaalgo)
{
     SysInfo* pSysInfo = GetSysInfo();

	 switch(pSysInfo->dmva_config.dmvaAppCfgCTDImdSmetaTzOc_sys) {
		 case 0:
			  strcpy(dmvaalgo, "Disable");
			  break;
		 case 1:
			  strcpy(dmvaalgo, "TAMPER");
			  break;
		 case 2:
			  strcpy(dmvaalgo, "IMD");
			  break;
		 case 4:
			  strcpy(dmvaalgo, "SMETA");
			  break;
		 case 8:
			  strcpy(dmvaalgo, "TZ");
			  break;
		 case 16:
			  ...
			  break;
		 case 3:
			  strcpy(dmvaalgo, "TAMPER,IMD");
			  break;
		 case 5:
			  strcpy(dmvaalgo, "TAMPER,SMETA");
			  break;
		 case 9:
			  strcpy(dmvaalgo, "TAMPER,TZ");
			  break;
		 case 17:
			  strcpy(dmvaalgo, "TAMPER,OC");
			  break;
		 default:
			  strcpy(dmvaalgo, "ERR...");
			  break;
...
int Curr_Param_List_To_html(void *pOutMem, int MaxSize)
{
    ...
    Get2AESwitch(String);
	sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","2A Engine", String, "TI");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));
    ...
    GetSNF(String);
	sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","Spatial Noise Filter", String, "Disable");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));
...
#ifdef _DMVAx_
	GetDMVAAlgo(String);
	sprintf(html_content,"A(\"%s\",\"%s\",\"%s\");","DMVA Algorithm", String, "TZ");
	if( MaxSize > strlen(pOutMem) )
		strncat(pOutMem, html_content, MaxSize-strlen(pOutMem));
#endif
