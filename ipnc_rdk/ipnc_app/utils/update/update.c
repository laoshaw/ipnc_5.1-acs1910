...
int FlashNand(FileInfo* pFile, char *pBlockName, unsigned long offset)
{
	...
	ret = SUCCESS;
	sprintf(cmd, "/usr/sbin/flash_eraseall  %s > %s/%s\n", pBlockName,
			TEMP_PATH, TEMP_FILE);
	if(system(cmd)){
...
//	char cmd[80];
	...
	int fd;
        ...
        char readbuff[3072], *strptr, *target;
	int flag = 0;
...
#endif
	/* get the current image env value from the env section of u-boot and
	based on that decide which partion we are in currently
	...
	fd = open("/dev/mtd0", O_RDONLY);
        ...
        size = lseek(fd, 0x240000, SEEK_SET);
        if (size == -1)
        {
                DBG("=======lseek on mtd0 failed========\n");
                return FAIL;
        }
        size = read(fd, readbuff, 3072);
        readbuff[3071] = '\0';
        strptr = readbuff;
        while (cnt < size) {
                length = strlen(strptr) + 1;
                if (strncmp(strptr, "currentimage=", 13) == 0)
                {
                        flag = 1;
                        break;
                }
	...
	close(fd);
        ...
        if (flag)
                strcpy(FILESYSBLOCK,"/dev/mtd3");
        else
                strcpy(FILESYSBLOCK,"/dev/mtd5");

        if ( FlashNand(pFile, FILESYSBLOCK, 0) == FAIL)
                return FAIL;

	/* update the env only if writing to nand succeeds */
        if (flag)
                system ("fw_setenv currentimage");
        else
                system ("fw_setenv currentimage 1");
