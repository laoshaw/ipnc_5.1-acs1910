...
int GenerateJpegFileName(char *strFileName)
{
	...
	strFileName[strlen(strFileName)] = '\0';
	return 0;
}
/**
...
void *JpgMngThr(void *arg)
{
	...
	while(!(GetJpgState(0) & JPG_THR_EXIT)){
		...
		if(gFileDispCount < FILE_DISP_THR_LIMIT){
			...
			// If restart flag is set then JPEG recording is ignored
			if(!IsAlarmRestart())
			    SaveJpeg(nSetFlg);
		} else{

			printf("Jpeg save limit nState = %d,gFileDispCount = %d\n",nState,gFileDispCount);
		}
#endif
	}
	JPG_DBG("Thread exit\n");
