						...
						int nStreamId, int nAudioBitrate)
{
	...
	if(ret != AVI_RUN_AGAIN){
		if(ret){
			...
			AVI_ERR("\n*** WARNING:Very small file recorded discarding it...\n");
			/* remove small file */
			sprintf(cmd, "rm -f %s\n", sAviName);
			AVI_DBG("%s\n",cmd);
...
int AviSave(int nDstFlg)
{
	...
	if (pSysInfo->lan_config.aviformat == 0) {
		...
		if(pSysInfo->lan_config.nDayNight == 0)
			nFrameRate = ((pSysInfo->lan_config.chipConfig == CHIP_DM365) || (pSysInfo->lan_config.chipConfig == CHIP_DMVA1)) ?
				enc_framerate_night_1_dm365[i][j][pSysInfo->lan_config.nFrameRate1]/1000:enc_framerate_night_1_dm368[i][j][pSysInfo->lan_config.nFrameRate1]/1000;
		else
			nFrameRate = ((pSysInfo->lan_config.chipConfig == CHIP_DM365) || (pSysInfo->lan_config.chipConfig == CHIP_DMVA1)) ?
				enc_framerate_day_1_dm365[i][j][pSysInfo->lan_config.nFrameRate1]/1000:enc_framerate_day_1_dm368[i][j][pSysInfo->lan_config.nFrameRate1]/1000;
	}else {
		...
		if(pSysInfo->lan_config.nVideocodecmode>7) {
			if(pSysInfo->lan_config.nDayNight == 0)
				nFrameRate = ((pSysInfo->lan_config.chipConfig == CHIP_DM365) || (pSysInfo->lan_config.chipConfig == CHIP_DMVA1)) ?
					enc_framerate_night_3_dm365[i][j][pSysInfo->lan_config.nFrameRate3]/1000:enc_framerate_night_3_dm368[i][j][pSysInfo->lan_config.nFrameRate3]/1000;
			else
				nFrameRate = ((pSysInfo->lan_config.chipConfig == CHIP_DM365) || (pSysInfo->lan_config.chipConfig == CHIP_DMVA1)) ?
					enc_framerate_day_3_dm365[i][j][pSysInfo->lan_config.nFrameRate3]/1000:enc_framerate_day_3_dm368[i][j][pSysInfo->lan_config.nFrameRate3]/1000;
		}
		else {
			if(pSysInfo->lan_config.nDayNight == 0)
				nFrameRate = ((pSysInfo->lan_config.chipConfig == CHIP_DM365) || (pSysInfo->lan_config.chipConfig == CHIP_DMVA1)) ?
					enc_framerate_night_2_dm365[i][j][pSysInfo->lan_config.nFrameRate2]/1000:enc_framerate_night_2_dm368[i][j][pSysInfo->lan_config.nFrameRate2]/1000;
			else
				nFrameRate = ((pSysInfo->lan_config.chipConfig == CHIP_DM365) || (pSysInfo->lan_config.chipConfig == CHIP_DMVA1)) ?
					enc_framerate_day_2_dm365[i][j][pSysInfo->lan_config.nFrameRate2]/1000:enc_framerate_day_2_dm368[i][j][pSysInfo->lan_config.nFrameRate2]/1000;
		}
	}
	/* Audio config */
...
void *AviMngThr(void *arg)
{
	...
	while(!(GetAviState(0) & AVI_THR_EXIT)){
		...
		AVI_DBG("Run\n");
		// If restart flag is set then AVI recording is ignored
		if(!IsAlarmRestart())
