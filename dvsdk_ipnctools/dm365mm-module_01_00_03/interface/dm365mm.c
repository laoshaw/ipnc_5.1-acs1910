...
void *DM365MM_mmap(unsigned long phyAddr,unsigned int size)
{
        void *virtAddr;
	FILE *dev_fp = NULL;
	...
	virtAddr = mmap(    (void   *)0, size, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_SHARED, dev_fp, phyAddr);
	close(dev_fp);

        return virtAddr;
}

int DM365MM_ummap(void *virtAddr,unsigned int size)
{
    if (dm365mm_fd == -1)
    {
        __E ("mmap: You must initialize DM365MM before making API calls.\n");
        return -1;
    }

    return(munmap(virtAddr,size));
}
