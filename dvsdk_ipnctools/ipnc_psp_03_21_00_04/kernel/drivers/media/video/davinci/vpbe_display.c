				...
				struct v4l2_format *fmt)
{
	...
	if (V4L2_BUF_TYPE_VIDEO_OUTPUT != fmt->type) {
		...
		if (pixfmt->pixelformat == V4L2_PIX_FMT_NV12
			 ...
			 */
			ret = osd_device->ops.request_layer
					(osd_device,otherlayer->layer_info.id);
			if (ret < 0) {
				/* Couldn't get layer */
				v4l2_err(&vpbe_dev->v4l2_dev,
...
static int vpbe_display_release(struct file *file)
{
	...
	disp_dev->fb_desc.cbcr_ofst = 0;
	disp_dev->fb_desc.yd_ofst = 0;
		...
		int cmd, void *param)
{
	...
	switch (cmd) {
	case VIDIOC_S_COFST:
		disp_dev->fb_desc.cbcr_ofst = *((unsigned long *) param);
		break;
	case VIDIOC_S_YDOFST:
		disp_dev->fb_desc.yd_ofst = *((unsigned long *) param);

		/* The Y-plane display offset must be 64-byte aligned */
		if (disp_dev->fb_desc.yd_ofst & 0x3F) {
			v4l2_err(&vpbe_dev->v4l2_dev, "fb_desc.yd_ofst is not "
				"aligned to 64 bytes\n");
			disp_dev->fb_desc.yd_ofst = 0;
			ret = -EINVAL;
		}
