...
const struct vpfe_standard_preset vpfe_standard_presets[] = {
	{{V4L2_DV_720P60}, 0, {1, 60} },
	{{V4L2_DV_1080I60}, 1, {1, 60} },
	{{V4L2_DV_1080I50}, 1, {1, 50} },
	{{V4L2_DV_720P50}, 0 , {1, 50} },
	{{V4L2_DV_1080P60}, 0 , {1, 60} },
	{{V4L2_DV_480P59_94}, 0 , {1, 60} },
	{{V4L2_DV_576P50}, 0 , {1, 50} },
};
...
static int vpfe_s_std(struct file *file, void *priv, v4l2_std_id *std_id)
{
	...
	/*
	 * just to make sure dont allow to skip frame
	 * reset the skip frame value
	 */
	video->std_info.frame_format = 1;
	video->skip_frame_count = 1;
	video->skip_frame_count_init = 1;
			 ...
			 struct v4l2_dv_preset *preset)
{
	...
	for (i = 0; i < ARRAY_SIZE(vpfe_standard_presets); i++) {
		if (vpfe_standard_presets[i].preset.preset == preset->preset) {
			video->std_info.frame_format =
				vpfe_standard_presets[i].frame_format;
			video->std_info.fps = vpfe_standard_presets[i].fps;
			break;
		}
	}
		       ...
		       struct v4l2_streamparm *parm)
{
	...
	if (video->std_info.frame_format) {
		v4l2_err(&vpfe_dev->v4l2_dev,
	...
	if (capparam->timeperframe.numerator !=
	    video->std_info.fps.numerator ||
	    capparam->timeperframe.denominator >
	    video->std_info.fps.denominator) {
		v4l2_err(&vpfe_dev->v4l2_dev,
	...
	video->timeperframe = capparam->timeperframe;
	video->skip_frame_count = video->std_info.fps.denominator/
					capparam->timeperframe.denominator;
	video->skip_frame_count_init = video->skip_frame_count;
	mutex_unlock(&video->lock);
	ret = 0;
out:
	return ret;
		       ...
		       struct v4l2_streamparm *parm)
{
	...
	capparam->timeperframe = video->timeperframe;
	return 0;
}
