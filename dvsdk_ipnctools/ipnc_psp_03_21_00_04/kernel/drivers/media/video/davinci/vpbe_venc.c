...
/*
 * venc_set_1080i25_internal - Setup 1080i25 in venc for dm365 only
 */
static int venc_set_1080i25_internal(struct v4l2_subdev *sd)
{
	struct venc_state *venc = to_state(sd);
	struct venc_platform_data *pdata = venc->pdata;
	...
	venc_modify(sd, VENC_VMOD, (HDTV_1080I << VENC_VMOD_TVTYP_SHIFT),
		    VENC_VMOD_TVTYP);
	venc_modify(sd, VENC_VMOD, VENC_VMOD_VENC, VENC_VMOD_VENC);
	venc_write(sd, VENC_XHINTVL, 440);
	venc_write(sd, VENC_DACTST, 0x0);
	return 0;
}
			    ...
			    struct v4l2_dv_preset *dv_preset)
{
	...
	} else if (dv_preset->preset == V4L2_DV_1080I25) {
		if (venc->pdata->venc_type == DM365_VPBE) {
			ret = venc_set_1080i25_internal(sd);
			vdaccfg_write(sd, 0x081141EF);
			return ret;
		}
	}
