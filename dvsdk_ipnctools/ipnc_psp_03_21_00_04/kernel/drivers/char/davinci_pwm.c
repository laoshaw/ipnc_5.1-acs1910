...
char *dm3xx_name[] = { "pwm0", "pwm1", "pwm2", "pwm3" };
...
static long pwm_unlocked_ioctl(struct file *file, unsigned int cmd,
	      unsigned long arg)
{
	int mode;
...
static int __init pwm_init(void)
{
    ...
    //printk("pwm_device_count = %d, pwm_minor_count = %d\n", pwm_device_count, pwm_minor_count);
	for (i = 0; i < pwm_device_count; i++) {
		for (j = 0; j < pwm_minor_count; j++) {
		    ...
		    //				"davinci_pwm%d", j);
            device_create(pwm_class, NULL, devno, NULL, "davinci_pwm%d", j);
			/*
			 * DM3XX has PWM3 IRQ at #28
			 */
			if (j == 3) {
				result =
					request_irq(IRQ_DM365_PWMINT3, pwm_isr,
						0/*SA_INTERRUPT*/,
						pwm_dev_array[j]->name,
						pwm_dev_array[j]);
			} else {
				result = request_irq(IRQ_PWMINT0 + j,
						pwm_isr, 0/*SA_INTERRUPT*/,
						pwm_dev_array[j]->name,
						pwm_dev_array[j]);
			}
...
static void __exit pwm_exit(void)
{
	...
	if (pwm_class != NULL) {
		...
		for (i = 0; i < size; i++) {
			...
			if (i == 3)
				free_irq(IRQ_DM365_PWMINT3, pwm_dev_array[i]);
			else
