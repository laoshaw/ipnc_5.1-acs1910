...
/* This section of code is added for IPNC boards. 
IPNC app checks for the link status using ioctl
so we need to defince structure to hold the data */ 
...
extern void davinci_ipnc_get_mac_addr(char *);
 ...
 */
#define EMAC_PRIV_MII_READ 22
static int emac_devioctl(struct net_device *ndev, struct ifreq *ifrq, int cmd)
{
	
	emac_drv_priv_ioctl priv_ioctl;
	struct emac_priv *dev = netdev_priv(ndev);
	struct phy_device *phydev = dev->phydev;
	
	if (!(netif_running(ndev)))
		return -EINVAL;

	/* TODO: Add phy  write and private statistics get feature */

	/* The switch case for EMAC_PRIV_MII_READ has beed added for IPNC boards. 
	   IPNC app checks for the link status using ioctl 
	   Ideally we need to read the phy register and return the link status to user */
	
	if (copy_from_user((char *)&priv_ioctl, (char *)ifrq->ifr_data,
  	...
  	switch (priv_ioctl.cmd)
	{
		case EMAC_PRIV_MII_READ :
		{
			...
			if (copy_from_user((char *)&phy_params,(char *)priv_ioctl.data,sizeof(emac_phy_params)))
				return -EFAULT;
			
			phy_params.data = phydev->link;
			if (copy_to_user((char *)priv_ioctl.data,(char *)&phy_params, sizeof(emac_phy_params)))
				return -EFAULT;
...
static int __devinit davinci_emac_probe(struct platform_device *pdev)
{
	...
	/* for ipnc board we need to get the eth addr from bootargs*/
	davinci_ipnc_get_mac_addr(emac_addr);
	...
	emac_str_to_ethaddr(pdata->mac_addr, emac_addr);
