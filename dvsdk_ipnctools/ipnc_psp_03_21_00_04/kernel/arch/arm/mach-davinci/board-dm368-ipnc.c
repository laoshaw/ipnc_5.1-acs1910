...
static int dm365_rmd_cs_rdk_150_wl_set_power(int slot, int power_on);

static struct davinci_mmc_config dm365rmd_wl_config = {
	.get_ro         = NULL,
	.get_cd         = NULL,
	.set_power      = dm365_rmd_cs_rdk_150_wl_set_power,
	.wires          = 4,
	.max_freq       = 20000000,
...
#define VENC_STD_ALL	V4L2_STD_NTSC
...
struct davinci_eeprom_info davinci_spi_sony_info = {
	.eeprom_size = 8192,
	.page_size = 32,
	.page_mask = 0x001F,
...
struct davinci_spi_config_t davinci_spi_sony_spi_cfg = {
	.wdelay		= 0,
	.odd_parity	= 0,
	.parity_enable	= 0,
	.wait_enable	= 0,
...
#define DM365_WLAN_IRQ          104
#define DM365_WLAN_ENABLE       55

static struct wl12xx_platform_data dm365_wl12xx_wlan_data __initdata = {
	.irq = -1,
	.board_ref_clock = WL12XX_REFCLOCK_38_XTAL,
	.platform_quirks = WL12XX_PLATFORM_QUIRK_EDGE_IRQ,
	.irq = IRQ_DM365_ADCINT +1,
};

static void dm365_wl12xx_set_platform_data(void)
{
	dm365_wl12xx_wlan_data.irq = gpio_to_irq(DM365_WLAN_IRQ);
	if (wl12xx_set_platform_data(&dm365_wl12xx_wlan_data))
		pr_err("Error setting wl12xx data\n");

	pr_debug("\nirq = %d\n\n", dm365_wl12xx_wlan_data.irq);

};

static void dm365_rmd_cs_rdk_150_wl_init(void)
{
	printk("dm365_rmd_cs_rdk_150_wl_init\n");

	davinci_cfg_reg(DM365_AEMIF_CE1);
	if (gpio_request(DM365_WLAN_ENABLE, "wlan_en") ||
		gpio_direction_output(DM365_WLAN_ENABLE, 0))
		pr_err("Error initializing the wl12xx enable gpio\n");

	davinci_cfg_reg(DM365_INT_PRTCSS_GIO0);
	if (gpio_request(DM365_WLAN_IRQ, "wl12xx_irq") ||
		gpio_direction_input(DM365_WLAN_IRQ))
		pr_err("Error initializing the wl12xx irq gpio\n");

	dm365_wl12xx_set_platform_data();
};
...
static int dm365_rmd_cs_rdk_150_wl_set_power(int slot, int power_on)
{
	...
	if (power_on) {
		gpio_set_value(DM365_WLAN_ENABLE, 1);
		mdelay(15);
		gpio_set_value(DM365_WLAN_ENABLE, 0);
		mdelay(1);
		gpio_set_value(DM365_WLAN_ENABLE, 1);
		mdelay(70);
	} else {
...
#ifdef CONFIG_WL12XX_PLATFORM_DATA
	/* setup wifi module */
	dm365_rmd_cs_rdk_150_wl_init();
	davinci_setup_mmc(1, &dm365rmd_wl_config);
#endif
