				   ...
				   uint8_t *buf, int page)
{
	...
	for (i = 0; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		...
		if (nand->ecc.prepad) {
			nand_read_buf_spare(mtd,oob,nand->ecc.prepad);
			oob += nand->ecc.prepad;
		}
		...
		if (nand->ecc.postpad) {
			nand_read_buf_spare(mtd, oob, nand->ecc.postpad);
				   ...
				   uint8_t *buf, int page)
{
	...
	nand_read_buf_spare(mtd, oob, mtd->oobsize);
				  ...
				  int page, int sndcmd)
{
	...
	nand_read_buf_spare(mtd, bufpoi, mtd->oobsize);
...
void nand_flash_ipnc_init(void)
{
	emifregs	emif_regs;
	emif_regs = (emifregs)DAVINCI_DM365_ASYNC_EMIF_CNTRL_BASE;
	emif_regs->AB1CR = 0x40800080; 	
}
