 ...
 */
XDAS_UInt32                 CrcNyuv;
IMP4HDVICPDEC_Params             params;
IMP4HDVICPDEC_DynamicParams      dynamicParams;
...
int main ()
{
    ...
    IMP4HDVICPDEC_Status             status;
    IMP4HDVICPDEC_InArgs             inArgs;
   ...
   iresStatus = (IRES_Status)RMAN_register(&IRESMAN_HDVICP,
    ...
    printf("TestApp_SetInitParams done %d \n\n");
        ...
        params.viddecParams.size     = sizeof(IMP4HDVICPDEC_Params);
        status.viddecStatus.size     = sizeof(IMP4HDVICPDEC_Status);
        inArgs.viddecInArgs.size     = sizeof(IMP4HDVICPDEC_InArgs);
        outArgs.viddecOutArgs.size                 = sizeof(IMP4HDVICPDEC_OutArgs);
        dynamicParams.viddecDynamicParams.size
                                     = sizeof(IMP4HDVICPDEC_DynamicParams);
       ...
       //HDVICP_initHandle(&hdvicpObj);
        ...
        if ((handle =
              (IALG_Handle)mp4VDEC_create
                            (
                             (const IMP4HDVICPDEC_Fxns  *)&MPEG4VDEC_TI_IMPEG4VDEC,
                             (const IMP4HDVICPDEC_Params *) &params)) == NULL
                            )
        {
...
#ifdef _ENABLE_IRES_EDMA3
{
 ...
 temp = RMAN_assignResources((IALG_Handle)handle,
    ...
    MPEG4VDEC_TI_IRES.activateAllResources((IALG_Handle)handle);
        ...
        mp4VDEC_control((IMP4HDVICPDEC_Handle)handle,
                         XDM_GETBUFINFO,
                        (IMP4HDVICPDEC_DynamicParams *)&dynamicParams,
                        (IMP4HDVICPDEC_Status *)&status
                       );
...
----------------------------------------------\n");
    ...
    profileStart(0);
        ...
        time_before_process = decoder_timeptr->tv_usec;
            ...
            retVal = mp4VDEC_decode((IMP4HDVICPDEC_Handle)handle,
                                    (XDM1_BufDesc *)&inputBufDesc,
                                    (XDM_BufDesc *)&outputBufDesc,
                                    (IMP4HDVICPDEC_InArgs *)&inArgs,
                                    (IMP4HDVICPDEC_OutArgs *)&outArgs
...
//#endif
    ...
    if (IRES_OK != temp) {
        ...
        mp4VDEC_delete ((IMP4HDVICPDEC_Handle)handle);
#endif
        if(frameCount == 0)
        {
