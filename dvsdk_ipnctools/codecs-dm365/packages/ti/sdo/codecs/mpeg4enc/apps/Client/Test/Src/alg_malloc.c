...
#include  "TestAppEncoder.h"    /*For custom allocation size for algorithm */
...
extern unsigned char *pInternalDataMemoryBase;
...
//#define EXTERNAL_DATA_MEM_SIZE    (14*1024*1024)

unsigned char  ExternalGlobalMem[EXTERNAL_DATA_MEM_SIZE];
unsigned char  *ExternalGlobalMemPool = ExternalGlobalMem;
unsigned int   externalDataMemorySize = EXTERNAL_DATA_MEM_SIZE;

unsigned char pInternalDataMemoryBase[ INTERNAL_DATA_MEM_SIZE];
unsigned char *pInternalDataMemory = pInternalDataMemoryBase;
unsigned int  internalDataMemorySize = INTERNAL_DATA_MEM_SIZE;
...
int allocateMemTabRequest(IALG_MemRec * memTab)
{
    if (memTab->space == IALG_EXTERNAL) {
        ...
        //     printf("externalDataMemorySize=%d\n",externalDataMemorySize);
...
int freeMemTabRequest(IALG_MemRec * memTab)
{
    if (memTab->space == IALG_EXTERNAL) {
        ...
        memTab->base = 0;
        pInternalDataMemory = pInternalDataMemoryBase;
