...
extern sTokenMapping sTokenMap_base[] ;
 ...
 ***********************************************************************
 */
static XDAS_Int32 ParameterNameToMapIndex (XDAS_Int8 *s ,int baseParamsOnly)
{
    XDAS_Int32 i = 0;
    if (baseParamsOnly==1)
    {
        while (sTokenMap_base[i].tokenName != NULL)
            if (0==strcmp (sTokenMap_base[i].tokenName, s))
