...
configFileCount = 0;
    ...
    pfInputYUVFile = fopen(bChangeDirForConfigFile, "rb");
        ...
        dynamicparams.LBRskipcontrol = 0;
...
//                if (dynamicparams.sliceSize <= 0)
                ...
                if (params.entropyMode || (params.encQuality == 0) || (params.encQuality == 1))
                   puiSadMVinfo = (XDAS_Int32*)outobj.bufs[1];

                fprintf(SadMvFilePtr, "Frame : %d\n", uiNumFrames);
                if (params.encQuality == 1)
                {
                  for(cntRow = 0; cntRow < (inobj.frameHeight >> 4); cntRow++)
                  {
                      cnt = cntRow * 5 * (inobj.frameWidth >> 4);
                      for(cntCol = 0; cntCol < 5 * (inobj.frameWidth >> 4); cntCol+=5)
                      {
                          fprintf(SadMvFilePtr, "Mvx0 = %d, Mvy0 = %d\n",
                         (short)(puiSadMVinfo[cnt + cntCol] & 0x0000FFFF), (puiSadMVinfo[cnt + cntCol] >> 16));
                          fprintf(SadMvFilePtr, "Mvx1 = %d, Mvy1 = %d\n",
                         (short)(puiSadMVinfo[cnt + cntCol + 1] & 0x0000FFFF), (puiSadMVinfo[cnt + cntCol + 1] >> 16));
                          fprintf(SadMvFilePtr, "Mvx2 = %d, Mvy2 = %d\n",
                         (short)(puiSadMVinfo[cnt + cntCol + 2] & 0x0000FFFF), (puiSadMVinfo[cnt + cntCol + 2] >> 16));
                          fprintf(SadMvFilePtr, "Mvx3 = %d, Mvy3 = %d\n",
                         (short)(puiSadMVinfo[cnt + cntCol + 3] & 0x0000FFFF), (puiSadMVinfo[cnt + cntCol + 3] >> 16));
                          fprintf(SadMvFilePtr, "SAD = %d\n", (*((XDAS_Int32 *)(puiSadMVinfo) + cnt + 4 + cntCol)));
                      }
                  }
                }
                else
