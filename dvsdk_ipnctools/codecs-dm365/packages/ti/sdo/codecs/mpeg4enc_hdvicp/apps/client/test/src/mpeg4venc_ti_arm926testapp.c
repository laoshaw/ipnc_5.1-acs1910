...
{\
	int i;\
	printf("packet no = %d",outargs.numPackets);\
	MEMUTILS_cacheInv(outargs.packetSize, outargs.numPackets*4);\
    for(i = 0;i <outargs.numPackets;i++)\
    printf("\nSize of packet %d\t= %d",i+1,outargs.packetSize[i]);\
...
NEXT_CONFIG:
    ...
    XDAS_Int32 uiSize_temp = uiExtWidth * uiExtHeight;
    ...
    outargs.packetSize = CMEM_alloc(((uiSize_temp >> 8) * sizeof(XDAS_Int32)),&memParams);
#else
    ...
    outargs.packetSize = malloc((uiSize_temp >> 8) * sizeof(XDAS_Int32));
#endif
    uiNumFrames = 0;
    //params.ME_Type = 0;
	//params.UMV     = 1;
                          ...
                          FILE               *pfInYUVFile)
{
	...
	else
	{
        ...
        for(uiLineCnt = uiPicHeight; uiLineCnt < uiExtHeight; uiLineCnt++)
        {
			memcpy(pubSubPictY, pubSubPictY - uiExtWidth, uiPicWidth);
			pubSubPictY += uiExtWidth;
		}
        ...
        for(uiLineCnt = (uiPicHeight >> 1); uiLineCnt < (uiExtHeight >> 1); uiLineCnt++)
        {
			memcpy(pubSubPictUV, (pubSubPictUV - (uiExtWidth >> 1)), (uiPicWidth >> 1));
			pubSubPictUV += (uiExtWidth >> 1);
		}
